...
### 4. 對外端點（固定網址＝填一次 Webhook URL 就好）
...
```
ngrok http --domain=xxxxx.ngrok-free.dev 8000   # 新版 agent 亦可用 --url=
```
...
```
Tunnel 本身免費（只有網域年費）；適合放進履歷講「用 Cloudflare Tunnel 讓地端服務安全對外、不開防火牆 port」。

> ⚠️ `cloudflared tunnel --url http://localhost:8000`（Quick Tunnel）是匿名臨時通道，每次重跑換一個隨機 `trycloudflare.com` 網址——測試可用，但要固定網址請走上面 A 或 B。
...
**推薦：`scripts\start_demo.ps1`** —— 一次帶起「更新快照 → 預熱模型 → 起 webhook（另開視窗）→ ngrok 固定網域對外」。搭配 ngrok reserved static domain，對外網址每次都一樣，LINE console 的 Webhook URL 填一次就好（見〈階段 5〉對外端點）。
...
# 離線重跑、快照仍新時可加 -SkipSync；要跳過預熱加 -NoWarm
```

> 腳本會等 `healthz` 就緒才開 tunnel，並提醒你 LINE 憑證是否載入。Ollama 為開機常駐服務，通常不需手動啟動（確認用 `curl http://localhost:11434/api/tags`）。

<details><summary>手動四步驟（不用腳本時）</summary>
